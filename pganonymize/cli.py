...
def get_arg_parser():
    parser = argparse.ArgumentParser(
        description="Anonymize data of a PostgreSQL database"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--list-providers",
        action="store_true",
        help="Show a list of all available providers",
        default=False,
    )
    parser.add_argument(
        "--schema",
        help="A YAML schema file that contains the anonymization rules",
        default=DEFAULT_SCHEMA_FILE,
    )
    parser.add_argument("--dbname", help="Name of the database")
    parser.add_argument("--user", help="Name of the database user")
    parser.add_argument(
        "--password", default="", help="Password for the database user"
    )
    parser.add_argument(
        "--host", help="Database hostname", default="localhost"
    )
    parser.add_argument("--port", help="Port of the database", default="5432")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Don't commit changes made on the database",
        default=False,
    )
    parser.add_argument(
        "--dump-file", help="Create a database dump file with the given name"
    )
    parser.add_argument(
        "--init-sql",
        help="SQL to run before starting anonymization",
        default=False,
    )
...
def main(args):
    ...
    for schema_name, tables in schemas.items():
        ...
        overwrite_source = tables.get(
            "overwrite_values_in_source_tables", False
        )
        overwrite_values_in_source_tables = (
            False if target_schema else overwrite_source
        )
        ...
        if args.init_sql:
            logging.info(
                "Executing initialisation sql {}".format(args.init_sql)
            )
        ...
        end_time = time.time()
        logging.info(
            "Anonymization of schema_name took {:.2f}s".format(
                end_time - start_time
            )
        )
    ...
    logging.info(
        "Anonymization took {:.2f}s".format(time.time() - start_exec_time)
    )
